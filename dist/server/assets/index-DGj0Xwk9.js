...
function calcScores(h, items = []) {
  ...
  const waterTreatScore = Math.min(treatmentPts / 5, 7);
  const waterMobileScore = h.jerry ? 3 : 0;
...
function buildStrategy(h, items) {
  ...
  if (dailyCal > 0 && scores.foodDays < 3) {
    ...
    const calNeeded = Math.round(daysNeeded * dailyCal / 1e3);
    actions.push({
      priority: "med",
      title: "Close the gap to 30-day food supply",
